  ...
  for (const dir of dirs) {
    ...
    for (const f of files) {
      ...
      try {
        ...
        if (meta) {
          meta.subagentUsage = await scanSubagents(path.join(dirPath, sessionId, 'subagents'));
          sessions.push(meta);
        }
      } catch {
...
async function scanFile(abs, sessionIdFromName) {
  ...
  const scan = hit && hit.scan && st.size >= hit.scan.offset
    ? hit.scan
    : { offset: 0, aiTitle: null, usage: {}, lastMsgId: null, lastModel: null };
...
function scanLine(line, scan) {
  if (line.includes('"type":"ai-title"')) {
    ...
    try {
      ...
      const u = scan.usage[model] || (scan.usage[model] = { input: 0, output: 0, cacheRead: 0, cacheCreation: 0 });
      u.input += m.usage.input_tokens || 0;
      u.output += m.usage.output_tokens || 0;
      u.cacheRead += m.usage.cache_read_input_tokens || 0;
      u.cacheCreation += m.usage.cache_creation_input_tokens || 0;
    } catch {
      /* ignore */
    }
  }
    ...
    try {
      ...
      if (!entry || entry.mtimeMs !== st.mtimeMs || entry.size !== st.size) {
        const scan = entry && entry.scan && st.size >= entry.scan.offset
          ? entry.scan
          : { offset: 0, aiTitle: null, usage: {}, lastMsgId: null };
        await incrementalScan(abs, st.size, scan);
...
module.exports = { scanAllTranscripts, groupByProject, sessionTitle, combinedUsage, mergeUsage };
