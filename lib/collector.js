...
class Collector {
  ...
  assemble() {
    ...
    for (const g of this.raw.transcriptGroups.values()) {
      for (const m of g.sessions) titleBySession.set(m.sessionId, sessionTitle(m).title);
    }
    ...
    const next = { quota: this.raw.quota, liveSessions, projects, weeklyCost, errors: this.state.errors.slice(-5) };
