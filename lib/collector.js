...
const { scanAllTranscripts, groupByProject, sessionTitle, combinedUsage, combinedDays, dailyCostSeries, subagentSummary } = require('./transcripts');
...
class Collector {
  ...
  statsSummary() {
    ...
    for (const g of this.raw.transcriptGroups.values()) {
      ...
      const projModels = {};
      for (const m of g.sessions) {
        sessionDays.push(combinedDays(m));
